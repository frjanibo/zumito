...
void timeout_playerFlash(Sprite* s) {
	...
	if(--playerFlashes > 0)
		set_timeout( 120UL, timeout_playerFlash, s);
}
...
void PlayerAnimEnds(Sprite* s){
	...
	if(s->a==1) { // reproducimos la anim de flash 4 veces
		if(s->b++ > 3) {
			s->a++;
			s->b=0;
			s->anim = anim_player_dead; // y cambiamos a la anim de death
		}
	} else
...
void ThePlayerUpdate(Sprite spr) {
        ...
        if(spr->a == 0) {
			...
			if( (spr->x&3)==0 && (spr->y&3)==0 ) spr->speed = 0;

			if( in_KeyPressed(k.up) && spr->y>0 && Room_canMove(room,spr->x,spr->y, DIRECTION_UP)) {
							if(spr->direction==DIRECTION_UP) spr->speed=1;
							else spr->direction=DIRECTION_UP;
			}
			if( in_KeyPressed(k.down) && spr->y<88 && Room_canMove(room,spr->x,spr->y, DIRECTION_DOWN)) {
							if(spr->direction==DIRECTION_DOWN) spr->speed=1;
							else spr->direction=DIRECTION_DOWN;
			}

			if( in_KeyPressed(k.right) && spr->x<120 && Room_canMove(room,spr->x,spr->y, DIRECTION_RIGHT) ) {
					if(spr->direction==DIRECTION_RIGHT) spr->speed=1;
					else spr->direction=DIRECTION_RIGHT;
			}
			if( in_KeyPressed(k.left) && spr->x>0 && Room_canMove(room, spr->x,spr->y, DIRECTION_LEFT) ) {
					if(spr->direction==DIRECTION_LEFT) spr->speed=1;
					else spr->direction=DIRECTION_LEFT;
			}

			if( in_KeyPressed(k.fire) && shoot_cooldown==0) {
					s =  Bullet_new(player);
					if( s ) {
							sprite_safe_add( s );
